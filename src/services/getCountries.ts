import { countriesPtBr as countries } from "../countries";
...
export const getLocalCountries = (countryName: string) => {
    if (!countryName) return [];
    return countries.filter((country: string) => country.toLowerCase().includes(countryName.toLowerCase())).filter((_: any, index: number) => index < 5)
}
